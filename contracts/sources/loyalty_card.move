...
public fun create_card(owner_name: String, ctx: &mut TxContext): LoyaltyCard {
...
entry fun mint_card(owner_name: String, ctx: &mut TxContext) {
    let card = create_card(owner_name, ctx);
    transfer::transfer(card, ctx.sender());
}

public fun earn_points(card: &mut LoyaltyCard, points_to_add: u64) {
    ...
    card.tier = if (card.points >= 500) { 2 } else if (card.points >= 100) { 1 } else { 0 };
