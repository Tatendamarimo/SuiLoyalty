...
function EditBrandModal({ brand, onEdited }: { brand: Membership; onEdited: () => void }) {
  ...
  async function update() {
    ...
    try {
      const res = await fetch(`/api/brands/${brand.brand_id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
          name: brand.brand_name, // keep original
  ...
  return (
    <>
      ...
      {open && (
        <div onClick={() => { setOpen(false); setError(""); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#0f1421", border: "1px solid #1f2937", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.5)" }}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #1f2937", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ fontSize: 16, fontWeight: 700, color: "#e2e8f0", margin: 0 }}>Edit Brand Profile</h2>
              <button onClick={() => setOpen(false)} style={{ background: "transparent", border: "none", color: "#64748b", cursor: "pointer", fontSize: 24 }}>&times;</button>
            </div>
            <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
                  ...
                  <span style={{ fontSize: 13, color: "#94a3b8", fontFamily: "monospace" }}>{color.toUpperCase()}</span>
                </div>
              </div>
              {error && <div style={{ color: "#fca5a5", fontSize: 12 }}>{error}</div>}
              <button onClick={update} disabled={loading} style={{ width: "100%", padding: "12px", background: loading ? "#334155" : brand.brand_color, border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 700, cursor: loading ? "wait" : "pointer" }}>
